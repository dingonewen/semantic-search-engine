...
static void SigintHandler(int) {
  g_done = 1;
}  // oarameter is unused, omitted to pass tidy-check
...
static void HandleClient(void* arg) {
  ...
  while (true) {
    ...
    if (r.method == "GET") {
      ...
      if (r.method == "PUT") {
        response =
            static_put(ctx->files_root, rel, r.body, true);  // PUT has body
      } else if (r.method == "POST") {
        response =
            static_put(ctx->files_root, rel, r.body, false);  // POST has body
...
int HttpServer::run(const std::string& initial_response_path) {
  ...
  int res = bind(listen_fd, reinterpret_cast<struct sockaddr*>(&address),
                 sizeof(address));
  if (res != 0) {
    std::cerr << strerror(errno) << '\n';
    close(listen_fd);
  ...
  while (
      !g_done) {  // loop exits when Ctrl+C is pressed and SIGNIT set g_done = 1
    ...
    // prints in run() right after accept() succeedsand before dispatching to
    // the threadpool
    ...
    m_pool.dispatch({HandleClient, new ClientCtx{client_fd, home_page,
                                                 m_files_root, &m_index}});
